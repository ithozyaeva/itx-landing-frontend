...
<template>
  <div class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 p-4 md:p-6 h-fit">
    <div class="flex items-start md:items-center gap-10 flex-col md:flex-row">
      <TgImage :username="mentor.tg" class="rounded-full self-center" width="160" height="160" />
      <div>
        <h3 class="text-xl font-semibold text-gray-900">
          {{ mentor.firstName }} {{ mentor.lastName }}
        </h3>
        <p v-if="mentor.occupation" class="text-gray-600 mt-1">
          {{ mentor.occupation }}
        </p>
        <div v-if="mentor.experience" class="mt-4">
    ...
    <div class="mt-4">
      <div v-if="mentor.profTags?.length > 0" class="flex flex-wrap gap-2">
        <span
      ...
      <div v-if="mentor.services?.length > 0" class="mt-4">
      ...
      <div v-if="mentor.contacts?.length > 0" class="mt-5">
        <hr class="mb-4">
        <div class="flex items-center gap-5 flex-wrap">
          ...
          >
            <TgIcon v-if="contact.type === ContactType.Telegram" class="h-7 w-7" />
            <MailIcon
              v-else-if="contact.type === ContactType.Email"
              class="h-7 w-7"
            />
            <PhoneIcon v-else-if="contact.type === ContactType.Phone" class="h-7 w-7" />
            <MessageSquare v-else-if="contact.type === ContactType.Other" class="h-7 w-7" />
          </a>
        </div>
      </div>
    </div>
